...
class Graph(object):
    ...
    def create_root_node(self, html_path: str):
        ...
        try:
            ...
            raise ParseError(
                "File {} not found in directory".format(html_path))
    ...
    def add_edge(self, id_node_from, id_node_to):
        # append tuple of id_node_from and id_node_to
        self.edges.append((id_node_from, id_node_to))
    ...
    def create_xpath(self, node_id=None):
        if node_id == None:
            xpath = [self.xpath_soup(self.nodes[n].bs4_node)
                     for n in self.nodes]
        else:
    ...
    def get_children(self, remove_non_informative=True):
        def is_non_informative(node_navigatable_string):
            ...
            if(node_navigatable_string.name is None):  # remove comment, non tag
                return True
            # remove script
            if(node_navigatable_string.name in ['script', 'nav', 'header', 'footer', 'noscript']):
                return True
